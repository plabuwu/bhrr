...
export default function App() {
  ...
  const YouTubePlayer = ({ videoId, isFading }) => {
    ...
    useEffect(() => {
      ...
      // For simplicity in this implementation, we'll use an iframe approach
      return () => {
        // Cleanup if needed
      };
    }, [videoId]);
            ...
            {videos.map((video, index) => {
              ...
              return (
                <div 
                  ...
                  className={`relative rounded-md overflow-hidden cursor-pointer flex-shrink-0 w-32 h-20 bg-black group transition-all duration-200 border-4 ${
                    currentIndex === index ? "border-blue-500" : "border-transparent hover:border-gray-500"
                  }`}
                >
                  {videoId ? (
                    <img 
